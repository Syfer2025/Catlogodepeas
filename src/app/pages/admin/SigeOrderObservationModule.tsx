...
import { Loader2, ChevronDown, ChevronRight, XCircle, Search, Plus, Pencil, Hash, Copy, Check, Info, ChevronUp, MessageSquareText } from "lucide-react";
...
export function SigeOrderObservationModule({ isConnected }: Props) {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const res = await api.sigeOrderObservationGet(token, sId.trim());
      setSearchResult(res);
    } catch (e: any) {
      setSearchError(e.message || "Erro ao buscar observacao.");
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      const token = await getAccessToken();
      const res = await api.sigeOrderObservationCreate(token, cId.trim(), body);
      setCreateResult(res);
    } catch (e: any) {
      setCreateError(e.message || "Erro ao cadastrar observacao.");
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      const token = await getAccessToken();
      const res = await api.sigeOrderObservationUpdate(token, uId.trim(), body);
      setUpdateResult(res);
    } catch (e: any) {
      setUpdateError(e.message || "Erro ao alterar observacao.");
    ...
    <div>
      <button onClick={() => setExpanded(!expanded)}
        ...
        <div className="text-left flex-1">
          <h4 className="text-gray-900" style={{ fontSize: "0.95rem", fontWeight: 700 }}>Pedidos Observacao</h4>
          <p className="text-gray-500" style={{ fontSize: "0.75rem" }}>Buscar, cadastrar e alterar observacoes do pedido — 3 endpoints</p>
        </div>
...
//   "observacao": ""
// }
// Todas as tags sao opcionais no body.`}</code>
              </pre>
            </div>
          )}
          ...
          {activeTab === "search" && (
            <div className="border border-gray-100 rounded-lg overflow-hidden">
              <div className="flex items-center gap-3 p-3 bg-gray-50/50">
                ...
                <code className="text-gray-800" style={{ fontSize: "0.85rem" }}>/order/{"{id}"}/observation</code>
              </div>
              <div className="p-3 space-y-3">
                <p className="text-gray-600" style={{ fontSize: "0.78rem" }}>
                  Busca a observacao de um pedido pela chave fato.
                </p>
                ...
                <span className="px-2 py-0.5 bg-green-50 text-green-700 border border-green-100 rounded"
                  style={{ fontSize: "0.65rem", fontWeight: 600 }}>Proxy: /sige/order/:id/observation</span>
                <button onClick={handleSearch} disabled={searching || !isConnected || !sId.trim()}
                  ...
                  {searching ? "Buscando..." : "Buscar Observacao"}
                </button>
                <ResultBlock result={searchResult} error={searchError}
                  label={`Resposta GET /order/${sId || "{id}"}/observation:`}
          ...
          {activeTab === "create" && (
            <div className="border border-gray-100 rounded-lg overflow-hidden">
              ...
              <div className="p-3 space-y-3">
                <p className="text-gray-600" style={{ fontSize: "0.78rem" }}>
                  Cadastra uma observacao no pedido pela chave fato.
                </p>
                <div className="relative">
                  <Hash className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-gray-400" />
                ...
                <button onClick={handleCreate} disabled={creating || !isConnected || !cId.trim()}
                  ...
                  {creating ? "Cadastrando..." : "Cadastrar Observacao"}
                </button>
                <ResultBlock result={createResult} error={createError}
                  label={`Resposta POST /order/${cId || "{id}"}/observation:`}
          ...
          {activeTab === "update" && (
            <div className="border border-gray-100 rounded-lg overflow-hidden">
              ...
              <div className="p-3 space-y-3">
                <p className="text-gray-600" style={{ fontSize: "0.78rem" }}>
                  Altera a observacao de um pedido pela chave fato.
                </p>
                <div className="relative">
                  <Hash className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-gray-400" />
                ...
                <div>
                  <p className="text-gray-500 mb-1.5" style={{ fontSize: "0.7rem", fontWeight: 600 }}>REQUEST BODY (JSON)</p>
                  <textarea value={updateJson} onChange={(e) => setUpdateJson(e.target.value)} rows={8}
                    className="w-full px-3 py-2.5 border border-gray-200 rounded-lg bg-gray-50 text-gray-800 font-mono outline-none focus:border-orange-400 focus:ring-2 focus:ring-orange-100 focus:bg-white transition-all"
                    style={{ fontSize: "0.75rem", lineHeight: 1.5 }} />
                </div>
                <button onClick={handleUpdate} disabled={updating || !isConnected || !uId.trim()}
                  ...
                  {updating ? "Alterando..." : "Alterar Observacao"}
                </button>
                <ResultBlock result={updateResult} error={updateError}
                  label={`Resposta PUT /order/${uId || "{id}"}/observation:`}

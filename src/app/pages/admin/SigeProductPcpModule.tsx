...
import { Loader2, ChevronDown, ChevronRight, XCircle, Search, Hash, Copy, Check, Info, ChevronUp, ClipboardList, Building2, Tag } from "lucide-react";
...
export function SigeProductPcpModule({ isConnected }: Props) {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const res = await api.sigeProductPcpGet(token, productId.trim(), params);
      setResult(res);
    } catch (e: any) {
      setError(e.message || "Erro ao buscar PCP.");
    ...
    <div>
      <button onClick={() => setExpanded(!expanded)}
        ...
        <div className="text-left flex-1">
          <h4 className="text-gray-900" style={{ fontSize: "0.95rem", fontWeight: 700 }}>Produto PCP</h4>
          <p className="text-gray-500" style={{ fontSize: "0.75rem" }}>Plano de Controle: custo, margem, classificacoes — 1 endpoint</p>
        </div>
...
//   codRef    -> código de referência`}</code>
          ...
          <div className="border border-gray-100 rounded-lg overflow-hidden">
            <div className="flex items-center gap-3 p-3 bg-gray-50/50">
              ...
              <code className="text-gray-800" style={{ fontSize: "0.85rem" }}>/product/{"{id}"}/product-control-plan</code>
            </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                <div className="relative">
                  ...
                  <input type="text" value={codFilial} onChange={(e) => setCodFilial(e.target.value)}
                    placeholder="codFilial (opcional)" className={inputClass} style={inputStyle} />
                </div>
                <div className="relative">
                  <Tag className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-gray-400" />
                  <input type="text" value={codRef} onChange={(e) => setCodRef(e.target.value)}
                    placeholder="codRef (opcional)" className={inputClass} style={inputStyle} />
                </div>
              </div>

              <div className="flex items-center gap-2">
                <span className="px-2 py-0.5 bg-green-50 text-green-700 border border-green-100 rounded"
                  style={{ fontSize: "0.65rem", fontWeight: 600 }}>Proxy: /sige/product/:id/product-control-plan</span>
              </div>
                ...
                <div className="bg-gray-900 rounded-lg overflow-hidden">
                  <div className="flex items-center justify-between px-3 pt-2.5 pb-1">
                    <p className="text-green-400" style={{ fontSize: "0.68rem", fontWeight: 600 }}>
                      Resposta GET /product/{productId}/product-control-plan:
                    </p>
